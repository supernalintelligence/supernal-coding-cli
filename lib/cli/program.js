...
function buildProgram() {
  ...
  // ============================================================================
  // GITHUB COMMAND - Sync GitHub issues, PRs, CI status
  // ============================================================================
  program
    .command('github <action>')
    .description('Sync GitHub issues, PRs, and CI status to local tracking')
    ...
    .action(async (action, options) => {
      try {
        ...
        switch (action) {
          ...
          default:
            console.error(chalk.red(`Unknown action: ${action}`));
            console.log('Available actions: sync, issues, prs, ci');
        }
      } catch (error) {
        console.error(chalk.red(`❌ ${error.message}`));
  ...
  // Test command (enhanced with evidence logging - REQ-106)
  program
    .command('test [action]')
    .description('Test management, execution, and evidence logging')
  ...
  program
    ...
    .option(
      '--template <type>',
      'Dashboard template type (default: dashboard-v2)'
    )
  ...
  // ============================================================================
  // RESOURCE COMMAND (External Integrations)
  // ============================================================================
  const resourceCommand = require('../../cli/resource');
  program.addCommand(resourceCommand.program);

  // ============================================================================
  // JIRA COMMAND (Jira Integration)
  // ============================================================================
  const jiraCommand = require('../../cli/jira');
