  ...
  async checkBranchSafety(branchName) {
    ...
    try {
      ...
      try {
        ...
        issues.push(`Branch '${branch}' does not exist`);
      ...
      // Check 3: Is branch up to date with main?
      try {
        ...
        if (parseInt(behindCount, 10) > 0) {
          issues.push(`Branch is ${behindCount} commit(s) behind main`);
        }
      } catch (_e) {
        // Might fail if no main branch
      }

      // Output results
      console.log(`\n🔍 BRANCH SAFETY CHECK: ${branch}\n`);
      if (issues.length === 0) {
        console.log('✅ Branch is safe to merge');
        return { safe: true, issues: [] };
      } else {
        console.log('⚠️  Issues found:');
        issues.forEach((issue) => console.log(`   - ${issue}`));
        return { safe: issues.every((i) => i.includes('behind')), issues };
      }
    } catch (error) {
      console.error(`❌ Error checking branch safety: ${error.message}`);
      return { safe: false, issues: [error.message] };
    }
  }
  ...
  try {
    switch (action) {
      ...
      case 'check-context': {
        ...
        console.log('\n🔍 WORK CONTEXT CHECK\n');
        console.log(`📋 Current Branch: ${gitSmart.getCurrentBranch()}`);
        console.log(`🔄 Recent Work Detected: ${detectedReqs.length ? detectedReqs.join(', ') : 'None'}`);
        if (contextCheck.status === 'error') {
          console.error(`\n🚨 ${contextCheck.message}`);
          process.exit(1);
        } else if (contextCheck.status === 'warning') {
