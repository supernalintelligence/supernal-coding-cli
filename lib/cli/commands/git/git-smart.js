...
/**
 * Handle git-smart command from program.js
 * @param {string} action - The action to perform (branch, merge, status, etc.)
 * @param {string} identifier - Optional identifier (e.g., branch name, req ID)
 * @param {Object} options - Command options
 */
async function handleGitSmartCommand(action, identifier, options = {}) {
  const gitSmart = new GitSmart();
  ...
  try {
    switch (action) {
      ...
      case 'branch': {
        const reqId = options.branch || identifier;
        if (!reqId) {
          console.error('❌ Usage: sc git-smart branch --branch REQ-XXX');
          console.error('   or: sc git-smart branch REQ-XXX');
          process.exit(1);
        }
        gitSmart.createBranch(reqId.toUpperCase());
        break;
      ...
      case 'check-branch': {
        const branchName = identifier || gitSmart.getCurrentBranch();
        const result = await gitSmart.checkBranchSafety(branchName);
        if (!result.safe) {
          process.exit(1);
        }
        break;
      ...
      case 'check-context': {
        ...
        if (contextCheck.status === 'error') {
          ...
          console.log('\n✅ Work context looks good');
        }
        break;
      }

      case 'merge': {
        const branchToMerge = identifier || gitSmart.getCurrentBranch();
        await gitSmart.performSafeMerge(branchToMerge, {
          push: options.push,
      ...
      case 'push': {
        // Push is handled by sync-push
        console.log('💡 Use "sc git-smart sync-push" for safe push operations');
        break;
      }

      case 'deploy': {
        await gitSmart.performDeployment({
          version: options.tag,
          skipTests: options.skipTests,
          skipLint: options.skipLint,
          pushTags: !options.noPushTags,
        });
        break;
      }

      case 'sync-push': {
        await gitSmart.syncPushSubRepos(options);
        break;
      }

      case 'subrepos':
      case 'subrepo-status': {
        await gitSmart.showSubRepoStatus(options);
        break;
      }

      case 'cleanup-branches': {
        await gitSmart.cleanupMergedBranches({
      ...
      case 'list-branches':
      case 'branch-status': {
        await gitSmart.listBranchStatus(options);
        break;
      }
      ...
      case 'check-compliance': {
        gitSmart.checkBranchCompliance(options);
        break;
      }
      ...
      default:
        ...
        console.log('  branch            Create requirement-linked branch');
        console.log('  check-branch      Check branch safety for merge');
        console.log('  check-context     Check work context');
        console.log('  check-compliance  Check branch naming compliance');
        console.log('  merge             Safe merge with validation');
        console.log('  deploy            Deploy with version tag');
        console.log('  sync-push         Sync all repos and push');
        console.log('  subrepos          Show subrepo status');
        console.log('  branch-status     Show all branch status');
        console.log('  cleanup-branches  Clean up merged branches');
        console.log('  suggest           Suggest branch naming');
        process.exit(1);
    }
  } catch (error) {
...
module.exports.handleGitSmartCommand = handleGitSmartCommand;
