...
class DashboardManager {
  ...
  findPackageDashboard() {
    ...
    if (this.projectRoot.includes('supernal-coding')) {
      const devDashboard = path.join(this.projectRoot, 'apps', 'dashboard-v2');
    ...
    try {
      ...
      const pkgDashboard = path.join(pkgRoot, 'apps', 'dashboard-v2');
    ...
    throw new Error(
      'Dashboard not found. Is supernal-coding installed? Tried:\n' +
        `  - Development: ${path.join(this.projectRoot, 'apps', 'dashboard-v2')}\n` +
  ...
  async init(options = {}) {
    try {
      ...
      const targetPath = path.join(this.projectRoot, 'apps', 'dashboard-v2');

      // Check if already exists
      if (await fs.pathExists(targetPath)) {
        if (!options.force) {
          console.log(
            chalk.red('\n❌ Dashboard already exists at apps/dashboard-v2/')
          );
      ...
      console.log(chalk.white(`   Location: apps/dashboard-v2/`));
      console.log(
      ...
      console.log(chalk.gray('      rm -rf apps/dashboard-v2/'));
...
`;
  ...
  async copyDashboardV2Template(dashboardDir, options = {}) {
    try {
      ...
      // Source: apps/dashboard-v2 in the supernal-coding repo
      const templatePath = path.join(this.projectRoot, 'apps', 'dashboard-v2');
...
<head>
    ...
    <script>
        // Redirect to dashboard-v2 (local or production)
        const isLocal = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
        const dashboardUrl = isLocal 
            ? 'http://localhost:3006/' 
            : 'https://dashboard-v2-supernal-coding.vercel.app/';
 ...
 * Generated by: sc dashboard init --template=dashboard-v2
 */
...
<body>
  ...
  async serve(options = {}) {
    try {
      ...
      const localDashboard = path.join(projectRoot, 'apps', 'dashboard-v2');
