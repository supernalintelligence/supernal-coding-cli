...
import { getStatusColor, truncate, formatDate } from './utils';
...
async function handler(args: string[]): Promise<SprintResult> {
  let [boardId] = args;
  ...
  try {
    ...
    if (!boardId) {
      ...
      const scrumBoards = (boards.values as JiraBoard[]).filter((b) => b.type === 'scrum');
      ...
      boardId = String(scrumBoards[0].id);
    ...
    const sprint: JiraSprint | undefined = sprints.values?.[0];
    ...
    console.error(chalk.red(`Failed to show sprint: ${(error as Error).message}`));
