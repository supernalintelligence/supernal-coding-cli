...
class CommandMapper {
  ...
  /**
   * Discover all commands by parsing cli/program.js (where commands are defined)
   */
  async discoverCommands() {
    const programPath = path.join(this.cliDir, 'program.js');
    const indexContent = await fs.readFile(programPath, 'utf8');

    const commands = [];

    // Parse .command() definitions using regex
    // Allow for .alias() or other chained methods between .command() and .description()
    // Limit match to ~200 chars to avoid crossing command boundaries
    const commandRegex =
      /program\s*\.command\(['"`]([^'"`]+)['"`]\)(?:[^p]{0,200}?)\.description\(['"`]([^'"`]+)['"`]\)/g;
    let match;
  ...
  generateCursorRulesContent(mapping) {
    ...
    // Group commands by category (updated to current command names)
    const categories = {
      'Core Workflow': ['requirement <action>', 'validate', 'priority <action>', 'feature <action>'],
      Development: ['docs <action>', 'test [action]', 'build', 'type-check'],
      'System Management': ['init [directory]', 'git-hooks <action>', 'git-smart <action>', 'git <action>'],
      'Git & GitHub': ['github <action> [subaction] [args...]', 'monitor [subcommand]'],
      Configuration: ['config <action> [args...]', 'rules <action>', 'template <action>', 'workflow <action>'],
      Integrations: ['connect [plugin] [command] [args...]']
    };
...
`;
  ...
  generateAgentOnboardTemplate(mapping) {
    ...
    // Updated to reflect current CLI command names
    const keyCommands = {
      onboarding: commands.find((c) => c.name === 'agent' || c.name.startsWith('agent')),
      tasks: commands.find((c) => c.name === 'requirement <action>' || c.name === 'kanban'),
      validation: commands.find((c) => c.name === 'validate'),
      feedback: commands.find((c) => c.name === 'github <action> [subaction] [args...]'),
      priority: commands.find((c) => c.name === 'priority <action>' || c.name === 'priority'),
      help: true // Commander provides built-in help via --help
    };
...
${
  keyCommands.tasks
    ? `
**\`sc requirement list\`** - View current requirements and priorities
**\`sc req new "Title" --epic=name --priority=high\`** - Create new requirement  
**\`sc req update <id> --status=done\`** - Mark requirement complete
`
    : ''
}
...
**\`sc priority matrix\`** - Show priority matrix view
`
    : ''
}
...
${
  keyCommands.feedback
    ? `
**\`sc github issue create "title"\`** - Create GitHub issue
**\`sc github pr list\`** - List pull requests
**\`sc github status\`** - Check GitHub status
`
    : ''
}
