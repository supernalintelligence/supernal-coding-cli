 ...
 * Moved from scripts/check-type-duplicates.cjs for self-contained package architecture
 *
 * Enhanced Type Duplication Checker with Auto-Generated Type Filtering
 *
 * USAGE:
 *   sc type-check                          # Full analysis
 *   sc type-check --pre-commit             # Pre-commit check (fails if duplications found)
 *   sc type-check --show-ignored           # Show ignored auto-generated types
 *   sc type-check --show-legitimate        # Show legitimate duplications only
 *   sc type-check --add-ignore=TypeName    # Get command to add type to ignore list
 *   sc type-check --add-legitimate=TypeName # Add type to legitimate duplications
 *   sc type-check --init-config            # Create .duplication-lint.json config file
 *
 * FEATURES:
 *   ✅ Filters out auto-generated types (Supabase, database.types.ts, etc.)
 *   ✅ Linting system for marking legitimate duplications as OK
 *   ✅ Shows real duplications that need fixing
 *   ✅ Provides sed commands for bulk fixes
 *   ✅ Tracks imports to show what needs updating
 *   ✅ Configurable ignore lists for different environments
 *   ✅ Inline comment support: // duplication-ignore
 *   ✅ Pattern-based rules for family components and utilities
 *   ✅ Environment bypass via .code.env file
 *
 * IGNORE PATTERNS:
...
function findAllExports() {
  ...
  console.log(
    `📊 Processed ${fileCount.processed} files (${fileCount.total} total)`
  );
...
function generateFixCommands(duplicates) {
  ...
  commands.push('# 🛠️ TYPE CONSOLIDATION COMMANDS');
  commands.push(
  ...
  for (const [name, locations] of highPriorityDuplicates) {
    commands.push(`# ═══ ${name} (${locations.length} definitions) ═══`);
  ...
  commands.push('# 🔧 GENERIC CONSOLIDATION PATTERNS:');
  commands.push('');
...
function generateSummaryReport(
  ...
  content += `- **🚨 Real Duplications:** ${stats.duplicateNames} names (${stats.duplicateInstances} instances)\n`;
  content += `- **⚠️ Warning Duplications:** ${stats.warningNames || 0} names (${stats.warningInstances || 0} instances) - concerning but not blocking\n`;
  content += `- **👥 Family Components:** ${stats.familyLegitimateNames || 0} names (${stats.familyLegitimateInstances || 0} instances) - legitimate across families\n`;
  content += `- **🔍 Legitimate Duplications:** ${stats.legitimateNames} names (${stats.legitimateInstances} instances)\n`;
  content += `- **✅ Ignored Auto-Generated:** ${stats.ignoredNames} names (${stats.ignoredInstances} instances)\n`;
  ...
  if (stats.duplicateNames === 0) {
    content += `## ✅ SUCCESS: No Real Duplications Found!\n\n`;
    content += `All real type duplications have been resolved.\n\n`;
  } else {
    // Top duplications
    content += `## 🚨 Top Duplications (by count)\n\n`;
  ...
  if (Object.keys(legitimateDuplicates).length > 0) {
    content += `## 🔍 Legitimate Duplications (marked as OK)\n\n`;
...
function generateReport(
  ...
  console.log(
    `   🚨 Real duplications: ${stats.duplicateNames} names (${stats.duplicateInstances} instances)`
  );
  console.log(
    `   ⚠️ Warning duplications: ${stats.warningNames || 0} names (${stats.warningInstances || 0} instances)`
  );
  console.log(
    `   👥 Family components: ${stats.familyLegitimateNames || 0} names (${stats.familyLegitimateInstances || 0} instances)`
  );
  console.log(
    `   🔍 Legitimate duplications: ${stats.legitimateNames} names (${stats.legitimateInstances} instances)`
  );
  console.log(
    `   ✅ Ignored (auto-generated): ${stats.ignoredNames} names (${stats.ignoredInstances} instances)`
  );
  console.log('');
  ...
  if (stats.duplicateNames === 0) {
    console.log('🎉 No real type duplications found!');
    console.log(`📄 Report: ${result.summaryPath}`);
    return 0;
  } else {
    console.log('🔴 Top Real Type Duplications:');
    console.log('');
    ...
    console.log('');
    console.log(`📄 Detailed reports: ${result.baseDir}/`);
    console.log(`📋 Summary: ${result.summaryPath}`);

    if (isPreCommit && !forceCommit) {
      console.log('');
      console.log(
        '❌ COMMIT BLOCKED - Fix real duplications before committing'
      );
      console.log(
        '💡 Review individual duplication files for specific fix instructions'
      );
      console.log(
        '💡 Use --force to commit anyway (e.g., when committing duplication tooling)'
      );
      return 1;
    }
    ...
    if (isPreCommit && forceCommit) {
      console.log('');
      console.log('⚠️  FORCE COMMIT - Allowing commit despite duplications');
      console.log(
        '🔧 This should only be used for committing duplication tooling or emergency fixes'
      );
    }
...
function checkCodeEnvBypass() {
  ...
  try {
    ...
    console.warn(`⚠️  Could not read .code.env file: ${error.message}`);
    return { bypassed: false };
  }
}
...
function main() {
  ...
  if (envBypass.bypassed) {
    console.log('🔧 Type checking DISABLED via .code.env');
    console.log(`   Setting: ${envBypass.line}`);
    console.log(`   File: ${envBypass.file}`);
    console.log('');
    console.log('💡 To re-enable type checking:');
    console.log('   • Edit .code.env and set DISABLE_TYPE_CHECKING=false');
    console.log('   • Or delete the .code.env file');
    console.log('   • Or remove the DISABLE_TYPE_CHECKING line');
    console.log('');
    console.log('✅ Bypassing type duplication check');
    return 0;
  }
  ...
  console.log(
    '🔍 Enhanced Duplication Checker: Types, Functions, Enums, Constants'
  );
  console.log('');
...
// CLI Interface for SC command system
async function cliMain(args) {
  // Handle both array and string args
  const processArgs = Array.isArray(args) ? args : process.argv.slice(2);

  // Set process.argv to match what main() expects
  process.argv = ['node', 'sc-type-check', ...processArgs];
...
// Also export the core functions for potential reuse
module.exports.findAllExports = findAllExports;
...
function loadDuplicationLintConfig() {
  ...
  try {
    ...
    console.warn(
      `⚠️  Could not load duplication lint config: ${error.message}`
    );
  }
...
function createLintingConfig() {
  ...
  console.log(`✅ Created duplication linting config: ${configPath}`);
  console.log('');
  console.log('📝 Edit this file to configure legitimate duplications');
  console.log('');
...
function addToLegitimateConfig(typeName) {
  ...
  try {
    if (fs.existsSync(configPath)) {
      ...
      console.log('⚠️  No config file found. Run --init-config first.');
      return;
    }
  } catch (error) {
  ...
  if (!config.legitDuplicationPatterns.familyComponents.includes(typeName)) {
    ...
    try {
      ...
      console.log(`✅ Added '${typeName}' to legitimate family components`);
      console.log(`📄 Updated: ${configPath}`);
    } catch (error) {
      console.log(`❌ Error writing config: ${error.message}`);
    }
  } else {
    console.log(`ℹ️  '${typeName}' is already marked as legitimate`);
  }
}
...
function generateESLintSuggestions(name, locations) {
  let content = `## 🔧 ESLint-Style Comment Solutions\n\n`;
  ...
  if (familyCount > 1) {
    content += `- 👥 **Family Component** (${familyCount} families) - Each family needs its own version\n`;
  }
  if (isTest) {
    content += `- ⚠️ **Test Pattern** - Concerning but not blocking\n`;
  }
  if (isUtil) {
    content += `- 🔍 **Utility Pattern** - Legitimate if package independence needed\n`;
  }
  content += `- 🚨 **Real Duplication** - Needs consolidation\n\n`;
...
function generateFileSpecificESLintCommand(location, _name) {
  let content = `#### 💡 Add ESLint Comment:\n`;
  content += `\`\`\`bash\n`;
...
function updateSpecificDuplications(typeNames, baseDir = null) {
  if (!baseDir) {
    const latestPath = 'reports/duplications/latest';
    if (!fs.existsSync(latestPath)) {
      console.log('❌ No existing reports found. Run full analysis first.');
      return false;
    }
  ...
  console.log(`🔄 Updating specific duplications in ${baseDir}`);

  // Load existing data
  const dataPath = `${baseDir}/data.json`;
  if (!fs.existsSync(dataPath)) {
    console.log('❌ No data.json found. Cannot perform incremental update.');
    return false;
  }
  ...
  for (const typeName of typeNames) {
    if (data.duplicates[typeName]) {
      ...
      console.log(`⚠️ ${typeName} not found in duplications data`);
    }
  }

  console.log(`✅ Updated ${updated} duplication files`);
  return true;
}
...
function showUsageHelp() {
  console.log(`
🔍 Enhanced Type Duplication Checker

USAGE:
  sc type-check [OPTIONS]
...
EXAMPLES:
  ...
  sc type-check --pre-commit
  
  # Force commit despite duplications (for tooling commits)
  sc type-check --pre-commit --force
  
  # Update specific type after adding ESLint comment
  sc type-check --update=WaitlistForm
  
  # Update multiple types at once
  sc type-check --update-types=WaitlistForm,HeroSection,Button
  
  # Initialize configuration
  sc type-check --init-config
  
  # Add type to legitimate duplications
  sc type-check --add-legitimate=HeroSection
...
function enhanceSummaryWithUpdateSuggestions(duplicates) {
  ...
  for (const typeName of topDuplicates.slice(0, 5)) {
    content += `sc type-check --update=${typeName}\n`;
  }
  content += `\n# Update multiple types at once:\n`;
  content += `sc type-check --update-types=${topDuplicates.slice(0, 5).join(',')}\n`;
  content += `\n# Re-run full analysis:\n`;
  content += `sc type-check\n`;
  content += `\`\`\`\n\n`;
