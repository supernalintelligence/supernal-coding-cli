...
async function performSafeCommit(files, message, options = {}) {
  ...
  try {
    ...
    // Step 3: Commit with WIP check bypass
    console.log(chalk.blue('📝 Committing...'));
    
    // Determine if this is an agent-initiated commit
    const isAgentCommit = options.auto || false;
    
    // Get signing flags (agent commits are unsigned by default)
    const signingManager = new SigningManager(process.cwd());
    const signingFlags = signingManager.getSigningFlags({ isAgentCommit });
    
    // Add [SC] tag ONLY for agent-initiated commits
    let finalMessage = message;
    if (isAgentCommit && !message.startsWith('[SC]')) {
      finalMessage = `[SC] ${message}`;
    }
    
    const commitCmd = `SC_SKIP_WIP_CHECK=true git commit ${signingFlags} -m "${finalMessage.replace(/"/g, '\\"')}"`;
    execSync(commitCmd, {
      encoding: 'utf-8',
      shell: true
    });
