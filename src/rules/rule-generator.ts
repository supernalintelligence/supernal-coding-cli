...
class RuleGenerator {
  ...
  buildCreationSection(metadata) {
    ...
    if (metadata._naming_pattern) {
      content += `# ✓ Generates correct filename pattern\n`;
    }
    if (metadata._directory_base) {
      content += `# ✓ Creates in correct directory: ${metadata._directory_base}/\n`;
    }
    content += `# ✓ Fills required frontmatter\n`;
    content += `# ✓ Creates required sections\n`;
    content += `# ✓ Validates naming pattern\n`;
    content += `\`\`\`\n\n`;
  ...
  buildCodeConventionsSection(metadata) {
    ...
    if (conventions.interface_naming) {
      content += `**Interfaces**:\n\`\`\`typescript\n`;
      content += `// ✅ DO: ${conventions.interface_naming.style}`;
    ...
    if (conventions.class_naming) {
      content += `**Classes**:\n\`\`\`typescript\n`;
      content += `// ✅ DO: ${conventions.class_naming.style}`;
    ...
    if (conventions.function_naming) {
      content += `**Functions**:\n\`\`\`typescript\n`;
      content += `// ✅ DO: ${conventions.function_naming.style}\n`;
    ...
    if (conventions.constant_naming) {
      content += `**Constants**:\n\`\`\`typescript\n`;
      content += `// ✅ DO: ${conventions.constant_naming.style}\n`;
   ...
   */
  async generateAll() {
    console.log(chalk.blue('🔍 Finding templates with metadata...\n'));
  ...
  if (all || args.length === 0) {
    generator
      ...
      .then((results) => {
        ...
        console.log(chalk.green(`  ✓ ${succeeded} rule(s) generated`));
        if (failed > 0) {
    ...
    generator
      ...
      .then((rule) => {
        console.log(chalk.green(`✓ Generated rule: ${rule.filename}`));
        if (dryRun) {
