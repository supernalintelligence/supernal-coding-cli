...
## Git-Native Change Tracking (Recommended)

> **Important**: This project uses **git-native change tracking** (ADR-002) as the primary audit trail for document changes. This section describes the recommended approach.

### Why Git-Native?

| What | How | Compliance |
|------|-----|------------|
| What changed | `git diff` | FDA/SOC2 compliant |
| When | Commit timestamp | Immutable audit trail |
| Who | Commit author | Identity verification |
| Why | Commit message | Business justification |
| Approval proof | Signed commits (GPG) | FDA 11.50 compliance |

### Document Registry

Controlled documents are declared in `.supernal/document-registry.yaml`:

```yaml
controlled:
  required:  # Must use signed commits
    - "docs/workflow/sops/**/*.md"
    - "docs/compliance/**/*.md"
    - "docs/architecture/**/*.md"
  tracked:   # Logged, not enforced
    - "docs/requirements/**/*.md"
    - "docs/features/**/*.md"
```

### CLI Commands for Change Tracking

```bash
# View document change history with signature status
sc doc history docs/workflow/sops/general/SOP-0.1.13-change-control.md

# Check if staged files modify controlled documents
sc doc check

# Manage document registry
sc doc registry list
sc doc registry check <file>

# List documents pending approval
sc workflow list-pending
```

### When to Use Signed Commits

**Required for controlled documents** (SOPs, compliance, architecture):
```bash
git commit -S -m "[DOC] Update SOP-0.1.13 change control process"
```
...
See [GPG Setup Guide](/docs/guides/gpg-signed-commits-setup.md) for detailed instructions.

### When CHG Documents Are Needed

CHG (Change) documents are **only required** for:

1. **Complex changes** affecting 10+ controlled files
2. **Cross-team coordination** requiring formal approval
3. **Breaking changes** to APIs, processes, or compliance

For simpler changes, **signed commits with descriptive messages** provide sufficient audit trail.

```bash
# Create CHG document when needed
sc change new "Major SOP restructuring" --type=documentation
# Creates: docs/changes/CHG-000003-major-sop-restructuring.md
```

### Commit Message Format

Use standardized commit messages for audit trail clarity:

```
[TYPE](SCOPE): Brief description (50 chars)

Body explaining what changed and why.
Link to requirements or issues if applicable.

REQ-XXX: Related requirement (if applicable)
```
...
## 31. Change Control & Impact Analysis (Traditional Process)

> **Note**: The following sections describe the traditional CHG document process. Use this for complex, multi-file changes or when formal approval workflows are required.
